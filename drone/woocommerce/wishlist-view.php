...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
    ...
    <tr>
        ...
        <th class="product-thumbnail"><?php echo esc_html__('Image','drone'); ?></th>

        <th class="product-name">
            <span class="nobr">
                <?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product name', 'drone' ) ) ?>
            </span>
        </th>
        ...
        <?php if( $show_price || $show_price_variations ) : $column_count ++; ?>
            <th class="product-price">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit price', 'drone' ) ) ?>
                </span>
            </th>
        <?php endif; ?>
        ...
        <?php if( $show_quantity ) : $column_count ++; ?>
            <th class="product-quantity">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', __( 'Quantity', 'drone' ) ) ?>
                </span>
            </th>
        <?php endif;?>
        ...
        <?php if( $show_stock_status ) : $column_count ++; ?>
            <th class="product-stock-status">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock status', 'drone' ) ) ?>
                </span>
            </th>
        <?php endif; ?>
        ...
        <?php if( $enable_drag_n_drop ): $column_count ++; ?>
            <th class="product-arrange">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', __( 'Arrange', 'drone' ) )?>
                </span>
            </th>
        <?php endif; ?>
    ...
    if( $wishlist && $wishlist->has_items() ) :
            ...
            if( $product && $product->exists() ) :
                ...
                <tr id="yith-wcwl-row-<?php echo $item->get_product_id() ?>" data-row-id="<?php echo $item->get_product_id() ?>">
                    ...
                    <td class="product-remove">
                        <div>
                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'drone' ) ); ?>">&times;</a>
                        </div>
                    </td>
                    <?php endif; ?>
                    ...
                    <?php if( $show_stock_status ) : ?>
                        <td class="product-stock-status">
                            <?php echo $stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', __( 'Out of stock', 'drone' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', __( 'In Stock', 'drone' ) ) . '</span>'; ?>
                        </td>
                    <?php endif ?>
                    ...
                    <?php if( $show_last_column ): ?>
                        <td class="product-add-to-cart">
                            ...
                            if( $show_dateadded && $item->get_date_added() ):
                                echo '<span class="dateadded">' . sprintf( __( 'Added on: %s', 'drone' ), $item->get_date_added_formatted() ) . '</span>';
                            endif;
                            ?>
                            ...
                            <?php if( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ): ?>
                                <?php if( 'select' == $move_to_another_wishlist_type ): ?>
                                    <select class="change-wishlist selectBox">
                                        <option value=""><?php _e( 'Move', 'drone' ) ?></option>
                                        <?php
                                ...
                                <?php else: ?>
                                    <a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
                                        <?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'drone' ) ) ?>
                                    </a>
                                <?php endif; ?>
                            <?php endif; ?>
                    ...
                    <?php if( $repeat_remove_button ): ?>
                        <td class="product-remove">
                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist remove" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title',__( 'Remove this product', 'drone' )); ?>">&times;</a>
                        </td>
                    <?php endif; ?>
        ...
        <tr>
            <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'drone' ) ) ?></td>
        </tr>
    <?php
    endif;
