	...
	<div id="comment-<?php comment_ID(); ?>" class="comment_container media">
		...
		<div class="comment-text media-body">
			<?php if ( $comment->comment_approved == '0' ) : ?>
				<p class="meta"><em><?php esc_html_e( 'Your comment is awaiting approval', 'drone' ); ?></em></p>
			<?php else : ?>
				<p class="meta">
					<?php
						if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' )
							if ( wc_customer_bought_product( $comment->comment_author_email, $comment->user_id, $comment->comment_post_ID ) )
								echo '<em class="verified">(' . esc_html__( 'verified owner', 'drone' ) . ')</em> ';
			...
			<?php if ( $rating && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) : ?>
				...
				<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating clear" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'drone' ), $rating ) ?>">
					<span style="width:<?php echo ( $rating / 5 ) * 100; ?>%"><strong itemprop="ratingValue"><?php echo trim($rating); ?></strong> <?php esc_html_e( 'out of 5', 'drone' ); ?></span>
				</div>
