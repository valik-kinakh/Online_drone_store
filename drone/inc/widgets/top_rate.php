...
if (class_exists('PostRatings')) {
    ...
    class Drone_Apus_Top_Rate_Widget extends Apus_Widget {
        public function __construct() {
            parent::__construct(
                // Base ID of your widget
                'apus_top_rate_widget',
                // Widget name will appear in UI
                esc_html__('Apus Top Rate Widget', 'drone'),
                // Widget description
                array( 'description' => esc_html__( 'The highest rated posts on your site', 'drone' ), )
            );
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php esc_html_e('Get most rated:', 'drone'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('sort')); ?>"><?php esc_html_e('Sort by:', 'drone'); ?></label>
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('sort')); ?>" name="<?php echo esc_attr($this->get_field_name('sort')); ?>">
                <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php esc_html_e('Overall bayesian rating (score)', 'drone'); ?></option>
                <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php esc_html_e('Average rating', 'drone'); ?></option>
                <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php esc_html_e('Number of votes', 'drone'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <?php esc_html_e('Descending', 'drone'); ?>
                </label>
                ...
                <?php esc_html_e('Ascending', 'drone'); ?>
                </label>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('date_limit')); ?>"><?php esc_html_e('Ignore posts older than:', 'drone'); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id('date_limit')); ?>" name="<?php echo esc_attr($this->get_field_name('date_limit')); ?>" type="text" value="<?php echo esc_attr( $instance['date_limit'] ); ?>" size="3" /> <?php esc_html_e('days', 'drone'); ?>
                <br />
                <small><?php esc_html_e('(0 to ignore date)', 'drone'); ?></small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Limit:', 'drone'); ?></label>
    ...
    register_widget( 'Drone_Apus_Top_Rate_Widget' );
}